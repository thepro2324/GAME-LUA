...
-- משתנים פנימיים
local flyConnection, bodyVelocity, bodyGyro, noclipConnection, infJumpConnection, autoResetConnection, staffConnection
...
function PlayerMod.toggleFly(state)
    ...
    bodyVelocity = Instance.new("BodyVelocity", hrp); bodyVelocity.MaxForce = Vector3.new(1e5, 1e5, 1e5); bodyVelocity.Velocity = Vector3.new(0, 0, 0)
...
function PlayerMod.toggleNoclip(state)
    ...
    if state then
        noclipConnection = RunService.Stepped:Connect(function()
            if lp.Character then for _, p in ipairs(lp.Character:GetChildren()) do if p:IsA("BasePart") then p.CanCollide = false end end end
        end)
    end
end
